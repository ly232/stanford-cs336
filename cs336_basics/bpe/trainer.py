...
class BpeTrainer:
    ...
    def persist(self, vocab_path: str | os.PathLike, merges_path: str | os.PathLike):
    ...
    def train(self, input_path: str | os.PathLike):
        with open(input_path, "r") as f:
            text = f.read()
        text_sequence, pretokens_counter = self.pretokenizer.pretokenize_for_training(
            text
        )
        ...
        merges = []
        initial_len = len(vocabs)
        to_create = max(0, self.vocab_size - initial_len)
        if to_create > 0:
            with tqdm(total=to_create, desc="Building vocab", unit="vocab") as pbar:
                while len(vocabs) < self.vocab_size:
                    ...
                    for pretoken in text_sequence:
                        ...
                        for tok1, tok2 in pairwise(text_sequence[pretoken]):
                            counter[(tok1, tok2)] += pretokens_counter[pretoken]
                    end_time = time.time()
                    profile["pretoken_pairwise_scan"].append(end_time - start_time)

                    start_time = time.time()
                    tok1, tok2 = sorted(
                        counter.keys(), key=lambda pair: (counter[pair], pair)
                    )[-1]
                    end_time = time.time()
            ...
            # nothing to do, already at or above requested vocab size
            pass
