...
def chat_loop(model: TransformerLanguageModel, bpe_codec: BpeCodec):
    ...
    for token in tqdm.tqdm(range(1000), desc="Generating text", unit="token"):
        next_token = (
            decoding.next_token(
                model,
                # Need to unsqueeze to add batch dimension. For inference, batch size is always 1.
                torch.tensor(tokens[-context_length:]).unsqueeze(0),
            )
            # Need to squeeze to remove batch dimension,
            # in order to convert to a list to be fed into BPE codec.
            .squeeze(0).tolist()
        )
